...
public class SlitherEntity extends Monster {
    ...
    @Override
    public void tick() {
        this.lerpedSlide= Mth.lerp(0.5f,this.lerpedSlide,getSlide());
        if(level() instanceof ServerLevel serverLevel){
            if(this.getSlide()>0){
                this.navigation.stop();
                this.addDeltaMovement(this.oldDir.scale(0.15*Math.pow(this.getSlide(),0.5)));
                this.setSlide(Math.max(getSlide()-0.03f,0.0f));
