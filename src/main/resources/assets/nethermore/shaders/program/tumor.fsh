...
void main(){
    ...
    fracted.x -= (fract(fracted.x*160.0)/160.0-0.5/160.0f)*Pixelation;
    fracted.y -= (fract(fracted.y*90.0)/90.0-0.5/90.0f)*Pixelation;
    ...
    dist/=3.0;
    dist*=FadeInTest;
    vec2 texEvil = fracted;

    vec2 texCopied = texCoord*(1.0-dist)+texEvil*dist;
    ...
    float noise = dist*((abs(cnoise(vec3(dirForNoise*0.7,GameTime/70.0f+20.0))*0.5)+0.2)*pow(length(fracted-vec2(0.5,0.5)),2.0)*2.0);
    float noise2 = dist*((abs(cnoise(vec3(dirForNoise,GameTime/50.0f+1000.0))*0.6)+0.2)*pow(length(fracted-vec2(0.5,0.5)),2.0)*2.0);

    //dist = clamp(dist/4.0*(0.8+length(texCopied-vec2(0.5,0.5))*0.2),1.0,20.0);
    //fragColor.xy/=dist*FadeInTest+(1.0-FadeInTest);
    fragColor.rgb*=(1.0f-Pixelation)+1.0f;
    fragColor.a=1.0f;
}
